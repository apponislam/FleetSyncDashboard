...
const UserSubscriptionTable = () => {
    ...
    const columns = [
        ...
        },
        {
            title: "Status",
            key: "status",
            render: (_, record) => <span className={`px-4 py-2 w-[120px] flex items-center justify-center rounded-lg text-white font-medium ${statusColor[record.status]}`}>{record.status}</span>,
        },
        {
            title: "Action",
            key: "action",
    ...
    return (
        <div className="p-4">
            ...
            >
                <Table className="rounded-xl overflow-hidden shadow-md" dataSource={userData} columns={columns} rowKey="key" pagination={{ pageSize: 8, position: ["bottomCenter"] }} />
